...
class ShapExplainer:
    ...
    def train_model(self, x_train, nb_epoch=200, batch_size=64):
        """
    ...
    def get_top_anomaly_to_explain(self, x_explain):
        ...
        # Erreur de prédiction au carré de chaque feature de chaque instance
        square_errors = np.power(x_explain - predictions, 2)
        # Erreur moyenne par ligne par instance
        mse_series = pd.Series(np.mean(square_errors, axis=1))
        # Ascending sort : from the biggest to the smallest mean squared errorr
        most_anomal_trx = mse_series.sort_values(ascending=False)
        # print(most_anomal_trx)
        columns = ["id", "mse_all_columns"]
        # For having squared error for each feature like 'squared_error_featureName' ...
    ...
    def explain_unsupervised_data(self, x_train, x_explain, autoencoder=None, return_shap_values=False):
        ...
        Args:
            ...
            return_shap_values (bool): If False, the resulting explnation featues set for each record will include only 
                                       ...
                                       the explanation features set will include the SHAP value of each feature in the explanation
                                       features set so the explanation features set will be composed of tupels of (str, float)
                                       when str will be the name of the explaining feature and float will be its SHAP value.
                                       Note that for the explained features (features with high reconstruction error), if they 
                                       did not appear in previous feature explanation (explanation of feature with higher 
                                       reconstruction error), they will not have any SHAP values. Therefore they get unique
                                       value of -1.
        ...
        for record_idx in top_records_to_explain:
            ...
            all_sets_explaining_features[str(record_idx)] = []
            ...
            for idx_explained_feature in range(num_of_features):
                ...
                all_sets_explaining_features[str(record_idx)].append(set_explaining_features)

            final_set_features = []
            final_set_items = []
            for item in sum(all_sets_explaining_features[str(record_idx)], []):
                if item[0] not in final_set_features:
                    final_set_features.append(item[0])
                    final_set_items.append(item)
                    
            if return_shap_values:
                all_sets_explaining_features[str(record_idx)] = final_set_items
            else:
                all_sets_explaining_features[str(record_idx)] = final_set_features

        with open('explanation.json', 'w') as jsonFile:
            json.dump(all_sets_explaining_features, jsonFile,indent=4)
